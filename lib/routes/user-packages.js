...
module.exports = function (imports) {
  ...
  app.put('/:username/:package/:file', function (req ,res) {
    ...
    Package.findByIdOrName(packageId, user.id, function (err, package) {
      if (err || !package) return res.send(404);
      
      package.updateFileContents(filename, content, function (err, updatedPackage) {
        if (err || !updatedPackage) return res.send(404);
