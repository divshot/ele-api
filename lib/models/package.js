...
var randomName = require('../util/random-name');
...
Package.methods.saveIfValid = function (done) {
  ...
  PackageModel.isNameUnique(package.name, package.user_id, function (err, isUnique) {
...
Package.methods.updateFilenamesWithPackageName = function (name) {
  var package = this;
  var newName = this.name;
  
  if (newName === name) return package;
  ...
  package.files = _.map(package.files, function (file) {
    ...
    if (truncFilename === name) file.name = newName + ext;
...
Package.pre('save', function (next) {
  ...
  // TODO: put this in it's own "pre" listener
