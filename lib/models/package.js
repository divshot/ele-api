...
Package.methods.updateFileContents = function (filename, content, done) {
  this.files = _.map(this.files, function (file, idx) {
    if (file.name == filename) file.content = content;
    return file;
  });
  
  this.save(function (err, package) {
    package.files = parseFileList(package.files);
    done(err, package);
  });
};
